...
function submitdata() {
 ...
 async function hashPIN(pin, secret) {
    ...
    const data = encoder.encode(pin + secret); // Add secret for salting
    const hashBuffer = await crypto.subtle.digest('SHA-256', data);
    const hashArray = Array.from(new Uint8Array(hashBuffer));
    const base64Hash = btoa(String.fromCharCode(...hashArray));
    return base64Hash.slice(0, 12); // Truncate to 12 characters
}
  let epin;
  const pin = document.getElementById('ind').value;
const secret = 'ym-secter-yek';

(async () => {
     epin = await hashPIN(pin, secret);
    ...
    formData4.append(`entry.1122259669`, epin);
...
localStorage.removeItem('formData');
  ...
  <tr>
    <td style="padding:20px; text-align:center; font-size:24px; font-weight:bold; color:#000000;">
      Save the Future
    </td>
  </tr>
  <tr>
    <td style="padding:15px 20px; font-size:14px; line-height:1.6; color:#333333;">
        A savings account offers a secure way to grow your money over time through interest, providing financial stability and future opportunities.
    </td>
  </tr>
  <tr>
    <td style="padding:15px 20px; background-color:#f5f7fa; font-weight:bold; text-align:center; font-size:14px; color:#000000;">
      Welcome to UP NEXT banking system
    </td>
  </tr>
  <tr>
    <td style="padding:20px; text-align:center; font-size:20px; font-weight:bold; color:#000000;">
      UP NEXT for Next generation
    </td>
  </tr>
  <tr>
    <td style="padding:15px 20px; font-size:14px; line-height:1.6; color:#333333;">
