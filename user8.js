...
if (t) {
    ...
    async function fetchData(t, n, l, r, o) {
        try {
            ...
            if (r === "balance4") {
                const match = trimmedContent.match(/-?\d+(\.\d+)?/);
 ...
 async function fetchAndProcessData() { 
    try {
        let response = await fetch(`${dgistart}/1VvKwtRmRSLy-eLCQfeCDeN6xT_vv-Gw5CsXbjcwcpxw/${dgih}`);
        let text = await response.text();
        let parser = new DOMParser();
        let doc = parser.parseFromString(text, "text/html");

        // Extract balance data
        let balanceCell = doc.querySelector("table").rows[1].cells[10];
        let balance = balanceCell.innerText || balanceCell.textContent;
        document.getElementById("balance4").innerText = balance.trim();

        // Extract another value
        let newValue = doc.querySelector("table tbody tr:first-child td:nth-child(6)").innerText;
        let previousValue = localStorage.getItem(storageKey);
...
localStorage.setItem(storageKey, newValue);
 localStorage.removeItem("popupShown");
    ...
    function xyz() {
        ...
        localStorage.setItem('payur', `sheetid=${user.sheetid}&formid=${user.formid}&sa=${user.sa}&sd=${user.sd}&sr=${user.sr}&name=${encodeURIComponent(user.name)}&id=${user.id}&mail=${user.mail}`);
        console.log(ex.mymail);
